...
- Public `pi-ccg` npm package name with both `pi-ccg` and `ccg` executable aliases.
- VitePress documentation for installation, configuration, workflow architecture, agents, CLI commands, and security boundaries.
- GitHub CI, Pages deployment, and npm release workflows for the Pi-only package.

### Changed

- Pi CLI is the only supervisor and dynamically launches the seven `ccg-*` agents with bounded fanout.
- Frontend, backend, and review/test model routes can be configured independently.
- Test failures and `Critical` findings are routed by `componentId` to the owning builder for at most two repair rounds.
- Update and version checks now resolve `pi-ccg@latest`.
- Contributor, security, issue, pull request, and repository documentation now describe the Pi-only architecture.

### Removed

- Legacy wrapper binary release automation and the Go wrapper CI gate from the active release process.
- Legacy Claude/Codex/Gemini runtime assets from the npm package surface.
