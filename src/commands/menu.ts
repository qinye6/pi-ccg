...
import { mergePiSettingsSubagents, mergeSubagentExtensionConfig } from '../utils/pi-config'
import {
  DEFAULT_PI_CAPS,
  getCcgMetadataPath,
  getPiAgentHome,
...
async function configureModels(): Promise<void> {
  ...
  const current = {
    frontendModel: currentModel(overrides, CCG_MODEL_AGENTS.frontendModel),
    backendModel: currentModel(overrides, CCG_MODEL_AGENTS.backendModel),
    reviewModel: currentModel(overrides, CCG_MODEL_AGENTS.reviewModel),
  }
  ...
  const answers = await inquirer.prompt<typeof current>([
    {
      type: 'input',
      name: 'frontendModel',
      message: 'Frontend / Mini Program builder 模型:',
  ...
  assign(CCG_MODEL_AGENTS.frontendModel, answers.frontendModel)
  assign(CCG_MODEL_AGENTS.backendModel, answers.backendModel)
  assign(CCG_MODEL_AGENTS.reviewModel, answers.reviewModel)

  await mergePiSettingsSubagents({ agentOverrides })
  ...
  while (true) {
    ...
    const { action } = await inquirer.prompt<{ action: string }>([{
      ...
      choices: [
        ...
        { name: '3. 配置 Frontend / Backend / Review 模型', value: 'models' },
        { name: '4. 配置 builder 与派生上限', value: 'limits' },
        { name: '5. Doctor 健康检查', value: 'doctor' },
        { name: '6. Status 安装概况', value: 'status' },
        { name: '7. 安全卸载 CCG managed Pi assets', value: 'uninstall' },
